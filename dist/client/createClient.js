...
exports.createClient = exports.ClientError = void 0
...
var ClientError = /** @class */ (function(_super) {
  ...
  function ClientError(message, errors) {
    ...
    if (Error.captureStackTrace) Error.captureStackTrace(_this, ClientError)
    return _this
  }
  return ClientError
})(Error)
...
exports.createClient = function(_a) {
  var fetcher = _a.fetcher,
    subscriptionCreatorOptions = _a.subscriptionCreatorOptions,
    queryRoot = _a.queryRoot,
    mutationRoot = _a.mutationRoot,
    subscriptionRoot = _a.subscriptionRoot,
    typeMapper = _a.typeMapper
  var createSubscription = subscriptionCreatorOptions
    ? getSubscriptionCreator_1.getSubscriptionCreator(subscriptionCreatorOptions)
    : function() {
        return rxjs_1.NEVER
      }
  var query = function(request) {
    ...
    return typeMapper
      ? resultPromise.then(function(result) {
          return applyTypeMapperToResponse_1.applyTypeMapperToResponse(queryRoot, result, typeMapper)
        })
      : resultPromise
  }
  var mutation = function(request) {
    ...
    var resultPromise = fetcher(
      requestToGql_1.requestToGql('mutation', mutationRoot, request, typeMapper),
      fetch,
      qs_1.default,
    )
    return typeMapper
      ? resultPromise.then(function(result) {
          return applyTypeMapperToResponse_1.applyTypeMapperToResponse(mutationRoot, result, typeMapper)
        })
      : resultPromise
  }
  var subscription = function(request) {
    ...
    var resultObservable = createSubscription(
      requestToGql_1.requestToGql('subscription', subscriptionRoot, request, typeMapper),
    )
    return typeMapper
      ? resultObservable.pipe(
          operators_1.map(function(result) {
            return applyTypeMapperToResponse_1.applyTypeMapperToResponse(subscriptionRoot, result, typeMapper)
          }),
        )
      : resultObservable
  }
  var mapResponse = function(path, defaultValue) {
    return function(response) {
      ...
      var result = lodash_1.get(response, __spreadArrays(['data'], path), defaultValue)
  ...
  return {
    ...
    chain: {
      query: chain_1.chain(function(path, request, defaultValue) {
        return query(request).then(mapResponse(path, defaultValue))
      }),
      mutation: chain_1.chain(function(path, request, defaultValue) {
        return mutation(request).then(mapResponse(path, defaultValue))
      }),
      subscription: chain_1.chain(function(path, request, defaultValue) {
        return subscription(request).pipe(operators_1.map(mapResponse(path, defaultValue)))
      }),
    },
  }
}
