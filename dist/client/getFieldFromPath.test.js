...
describe('getFieldFromPath', function() {
  test('gets nested field definition', function() {
    ...
    expect(getFieldFromPath_1.getFieldFromPath(typeMap['Some'], ['other', 'some', 'other', 'scalar'])).toBe(
      typeMap.Other.fields.scalar,
    )
  ...
  describe('throws an error', function() {
    test('when root is not provided', function() {
      return expect(function() {
        return getFieldFromPath_1.getFieldFromPath(undefined, ['some'])
      }).toThrow('root type is not provided')
    })
    test('when the path is empty', function() {
      return expect(function() {
        return getFieldFromPath_1.getFieldFromPath({ name: 'Root' }, [])
      }).toThrow('path is empty')
    })
    test('when requesting a field on a type without fields', function() {
      return expect(function() {
        return getFieldFromPath_1.getFieldFromPath({ name: 'Root' }, ['field'])
      }).toThrow('type `Root` does not have fields')
    })
