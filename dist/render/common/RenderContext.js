...
var RenderContext = /** @class */ (function() {
  ...
  RenderContext.prototype.addImport = function(from, isDefault, module, fromAbsolute, noAlias) {
    ...
    var existing = imports.find(function(i) {
      return (isDefault && i.isDefault) || (!isDefault && i.module === module)
    })
  ...
  RenderContext.prototype.getImportBlock = function() {
    ...
    Object.keys(this.imports).forEach(function(from) {
      var defaultImport = _this.imports[from].find(function(i) {
        return i.isDefault
      })
      var namedImports = _this.imports[from].filter(function(i) {
        return !i.isDefault
      })
      var statements = []
      if (defaultImport) {
      ...
      if (namedImports.length > 0) {
        statements.push(
          '{' +
            namedImports
              .map(function(i) {
                return i.alias ? i.module + ' as ' + i.alias : i.module
              })
              .join(',') +
            '}',
        )
  ...
  RenderContext.prototype.toCode = function(parser) {
    var blocks = __spreadArrays(this.codeBlocks)
