...
var renderTypeGuard = function(target, possible) {
  return (
    '\n  const ' +
    target +
    '_possibleTypes = [' +
    ...
    ']\n  export const is' +
    target +
    ' = (obj: { __typename: String }): obj is ' +
    target +
    " => {\n    if (!obj.__typename) throw new Error('__typename is missing')\n    return " +
    target +
    '_possibleTypes.includes(obj.__typename)\n  }\n'
  )
}
exports.renderTypeGuards = function(schema, ctx) {
  for (var name_1 in schema.getTypeMap()) {
    ...
    if (graphql_1.isUnionType(type)) {
      var types = type.getTypes().map(function(t) {
        return t.name
      })
      ctx.addCodeBlock(renderTypeGuard(type.name, types))
    } else if (graphql_1.isInterfaceType(type)) {
      var types = schema.getPossibleTypes(type).map(function(t) {
        return t.name
      })
      ctx.addCodeBlock(renderTypeGuard(type.name, types))
